                ...
                'Instances': [
                    {
                        ...
                        'State': {'Name': 'running'},
                        'LaunchTime': kwargs.get('LaunchTime', datetime.now(UTC.utc)),
...
def test_instance_idle__check_not_idle():
    ...
    with patch('boto3.client') as mock_client:
        mock_client.return_value.describe_instances.side_effect = [mock_describe_instances(LaunchTime=datetime.now(UTC.utc) - timedelta(days=8))]
...
def test_instance_idle__skip_cluster():
    ...
    environment_variables.environment_variables_dict['policy'] = 'instance_idle'
    with patch('boto3.client') as mock_client:
        mock_client.return_value.describe_instances.side_effect = [mock_describe_instances(Tags=tags, LaunchTime=datetime.now(UTC.utc) - timedelta(days=8))]
        instance_idle = InstanceIdle()
        response = instance_idle.run()
        assert len(response) == 0
...
def test_instance_idle__dryrun_no_delete():
    ...
    with patch('boto3.client') as mock_client:
        mock_client.return_value.describe_instances.side_effect = [
            mock_describe_instances(Tags=tags, LaunchTime=datetime.now(UTC.utc) - timedelta(days=8))]
        mock_client.return_value.get_metric_data.side_effect = [
