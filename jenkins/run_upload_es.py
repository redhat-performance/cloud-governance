...
es_index_perf = 'cost-explorer-perf'
es_index_psap = 'cost-explorer-psap'
es_index_perf_scale = 'cost-explorer-perf-scale'
es_index_global = 'cost-explorer-global'
...
os.system(f"""sudo podman run --rm --name cloud-governance -e account='perf-scale' -e policy=cost_explorer -e AWS_ACCESS_KEY_ID={AWS_ACCESS_KEY_ID_DELETE_PERF_SCALE} -e AWS_SECRET_ACCESS_KEY={AWS_SECRET_ACCESS_KEY_DELETE_PERF_SCALE} -e es_host={ES_HOST} -e es_port={es_index} -e es_index={es_index_global} -e cost_explorer_tags="{cost_tags}" -e granularity={granularity} -e cost_metric={cost_metric} -e log_level=INFO quay.io/ebattat/cloud-governance:latest""")
