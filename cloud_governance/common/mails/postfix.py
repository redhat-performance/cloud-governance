...
class Postfix:
    ...
    def prettify_to(self, to: Union[str, list]):
        ...
        if isinstance(to, list):
            allowed_domains = self.__environment_variables_dict.get('ALLOWED_EMAIL_DOMAINS', ['@redhat.com'])
            return ','.join([item if self._has_allowed_domain(item, allowed_domains) else f'{item}@redhat.com'
                             for item in to])
        return to

    @staticmethod
    def _has_allowed_domain(email: str, allowed_domains: list):
        """
        This method checks if email already ends with one of the allowed domains
        :param email:
    ...
    def prettify_cc(self, cc: Union[str, list], to: str = ''):
        ...
        allowed_domains = self.__environment_variables_dict.get('ALLOWED_EMAIL_DOMAINS', ['@redhat.com'])
        cc_unique_values = [cc_user if self._has_allowed_domain(cc_user, allowed_domains) else f'{cc_user}@redhat.com'
                            for cc_user in cc]
