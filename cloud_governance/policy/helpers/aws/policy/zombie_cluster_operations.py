...
class ZombieClusterOperations(AWSPolicyOperations):
    ...
    def __get_all_cluster_instances(self):
        """
        This method returns the non-terminated EC2 instances
        :return:
        """
        cluster_ids = set()
        non_terminated_instances = self._ec2_operations.get_all_non_terminated_instances()
        for instance in non_terminated_instances:
            for item in instance:
                for tag in item.get('Tags', []):
                    if tag['Key'].startswith(self._cluster_prefix):
                        cluster_ids.add(tag.get('Key'))
    ...
    def _delete_resource(self, resource_id: list):
        ...
        if resource_id:
            try:
                if 'vol' in resource_id[0]:
                    self._ec2_operations.delete_volumes(resource_ids=resource_id)
                    return "volumes are deleted"
            except Exception as err:
                raise err
    ...
    def process_and_delete_resources(self, zombie_cluster_resources: dict,
        ...
        for zombie_cluster_tag, resources in zombie_cluster_resources.items():
            ...
            zombie_cluster_resources_response[zombie_cluster_tag] = {
                ...
                'DaysCount': self.get_clean_up_days_count(tags),
                ...
                'CreateDate': str(found_create_date),
            }
